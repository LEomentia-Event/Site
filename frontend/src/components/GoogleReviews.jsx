...
const GoogleReviews = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch(() => {
        if (active) setData({ configured: false, reviews: [] });
      });
    return () => {
      active = false;
    };
  ...
  // Render nothing until loaded, or if not configured / no reviews
  if (!data || !data.configured || !data.reviews || data.reviews.length === 0) {
    return null;
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.reviews.map((r, idx) => (
            <a
              key={idx}
              href={r.google_maps_uri || data.google_maps_uri}
              target="_blank"
              rel="noopener noreferrer"
              data-testid={`google-review-${idx}`}
              className="bg-white border border-coffee/10 rounded-lg p-6 flex flex-col hover:shadow-lg transition-shadow duration-300"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-poppins text-xs text-coffee/50">
                    {r.relative_time}
                  </p>
                </div>
